...
if (engine::isPost())
{
	...
	$product->update($_POST['title'], $_POST['notes']);
	...
	foreach ($allPrices as $priceInfo)
	{
		...
		$includeTax = $_POST['price_tax'][$priceTypeID];
		if ($includeTax)
			$priceValue = round($priceValue / (1 + $taxRates[$_POST['tax_rate']]['rate']), 2);
...
foreach ($allPrices as $priceInfo)
{
	...
	$fRow = getSelectFormRow($form, $typeResource = $priceTypes, $typeValue = $priceInfo['type_id'], $valueValue = $priceInfo['price'], $class = "fprice", $ph, $name = "price", $id = $priceInfo['type_id'], $removable = TRUE, $attr);
	DOM::append($group, $fRow);
}
...
// Notes group
$title = $appContent->getLiteral("products.details.edit", "hd_notes");
$group = getEditGroup($title, $newButton = FALSE);
$form->append($group);

$textarea = $form->getTextarea($name = "notes", $value = $productInfo['notes'], $class = "ftext", $autofocus = FALSE, $required = FALSE);
DOM::append($group, $textarea);
$ph = $appContent->getLiteral("products.details.edit", "hd_notes", array(), FALSE);
...
function getSelectFormRow($form, $typeResource, $typeValue, $valueValue, $class, $ph, $name, $id = "", $removable = FALSE, $attr = array())
{
	// Create a new row
	$fRow = DOM::create("div", "", "", "frow");
	...
	if ($attr['tax'])
	{
		$input = $form->getInput($type = "checkbox", $name."_tax[$id]", $value = "0", $class = "fradio", $autofocus = FALSE, $required = FALSE);
		DOM::append($fRow, $input);
		$inputID = DOM::attr($input, "id");
		$labelTitle = appLiteral::get("products.details.edit", "lbl_price_includeTax");
		$label = $form->getLabel($labelTitle, $for = $inputID, $class = "frlabel");
		DOM::append($fRow, $label);
	}
